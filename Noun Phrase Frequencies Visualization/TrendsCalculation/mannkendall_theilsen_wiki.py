...
def drop_common_rare_phrases(joined_df):
    ...
    joined_df.drop(joined_df[joined_df.total_occurrences<200].index, inplace=True)
    return joined_df
...
def calculate_theilsen_slope(df, year_columns):
    """ Calculates the Theil sen slope of 11 years' doc frequencies for each phrase, and 95% confidence intervals. These are added as columns
    to the dataframe"""
...
def calculate_mannkendall_statistic(df, year_columns):
    """ Calculates the Mann Kendall statistic of 11 years' doc frequencies for each phrase at alpha = 0.05. The Mann Kendall Z is added as a
...
def create_sorted_stats_dfs(df):
    """ Takes a dataframe with phrase as index, percentage docs of all years, 3 Mann-Kendall statistic-related columns, and
...
def main():
    ...
    try:
        ...
        # Write to files
        subfolder = 'mannkendall_and_theilsen_wikipedia'
