""" Module to create files with positively trending and negative trending noun phrases between a start year
and an end year. 11 sets of 4 files each are created, where the end year is always 2017, but the start year
goes from 2007 to 2016."""
...
def grouped_dataframes_for_year(year):
    """ Takes a year as argument, queries the Solr index nounphraes on the published_date field using
    ...
    docs = search_solr_parse_json(year, 'nounphrases', 'published_date')
...
def normalize_dataframes(grouped_df, year):
    ...
    phrase_count, doc_count = get_counts_from_json(year)
    grouped_df['percentage_occurrences'] = grouped_df['total_occurrences'] * 100 / phrase_count
...
def main():
    ...
    pickle_names = {year: 'Pickles/'+ year + '_grouped_df.pickle' for year in years}
    ...
    total_docs_occurrences = pd.read_pickle('Pickles/total_doc_count_dataframe.pickle')
    ...
    for year in years[:-1]:
        ...
        joined_df = join_dfs_on_phrase(year_dataframe_dict.get(year), year_dataframe_dict.get('2017'), year)
