...
def drop_common_rare_phrases(joined_df):
    ...
    docs in which they occur, and drops phrases which are common (>50000 total occurrences) Returns smaller dataframe. """
   
    path_to_insert = "Output/SparsityStatsWiki"
    if not os.path.exists(path_to_insert):
        os.makedirs(path_to_insert)
    # unused_df = joined_df.loc[(joined_df.total_occurrences>50000) | (joined_df.total_occurrences<100) ]
    unused_df = joined_df.loc[(joined_df.total_occurrences<100)]
    #print(joined_df.loc[joined_df.total_occurrences>50000])
    unused_df.to_csv('{}/below100.tsv'.format(path_to_insert, encoding='utf-8-sig'), sep='\t')
    # Drop rows which have total_occurrences greater than 50000: common words and <100: rare words
    #joined_df = joined_df.drop(joined_df[joined_df.total_occurrences>50000].index)
    joined_df = joined_df.drop(joined_df[joined_df.total_occurrences<100].index)
    joined_df.to_csv('{}/Above100.tsv'.format(path_to_insert), sep='\t', encoding='utf-8-sig')
    return joined_df
...
def remove_more_phrases_and_sort(ready_for_stats_df, year_columns):
    ...
    lessthan3years_df.to_csv('{}/lessthan3years_and_above100.tsv'.format(path_to_insert), sep='\t', encoding='utf-8-sig')
    ...
    ready_for_stats_df.to_csv('{}/3ormoreyears_and_above100.tsv'.format(path_to_insert), sep='\t', encoding='utf-8-sig')
